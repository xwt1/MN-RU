...
#
#     # 保存为 PDF
#     output_path = "recall-time-isolated.pdf"
...
if __name__ == "__main__":
    ...
    # 创建图形和网格
    fig = plt.figure(figsize=(20, 5))  # 调整图形的大小，使其适合一排
    gs = GridSpec(1, 4, figure=fig, hspace=0.3, wspace=0.3)  # 修改为1行4列
    ...
    for idx, workload in enumerate(workloads):
        for j, dataset in enumerate(datasets):
            ax = fig.add_subplot(gs[0, idx * 2 + j])  # 计算每个子图的索引
            configure_axes(ax)
            ax.set_title(f"{workload} - {dataset}", fontsize=16, fontweight='bold')
    ...
    # 调整全局标签位置
    fig.text(0.5, -0.01, 'Query Time (ms)', ha='center', fontsize=18, fontweight='bold')
    fig.text(0.08, 0.5, 'Recall (%)', va='center', rotation='vertical', fontsize=18, fontweight='bold')

    # 全局图例
    fig.legend(line_handles, line_labels, loc='lower center', bbox_to_anchor=(0.5, 0.95), ncol=3, fontsize=12)
